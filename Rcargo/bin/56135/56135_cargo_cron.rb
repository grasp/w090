...
logger=Logger.new("56135_cargo.log", shift_age = 30, shift_size = 1048576)
logger.level = Logger::DEBUG
#logger.level = Logger::WARN 

#main program here
Forever.run do  
   logger.info "56135 cargo cron start successfully on #{Time.now}!"
   every 5.minutes do
     all_raw_cargo=run_56135_cargo_cron(logger)    
    
    if (all_raw_cargo||"").size==0 
      logger.warn "56135 cargo cron got no any cargo #{Time.now}!"
    end 
  ...
  on_error do |e|    
    logger.ERROR "56135 cargo cron Boom raised: #{e.message} #{Time.now}!"
  end

  on_exit do
    logger.ERROR "56135 cargo cron Bye bye on #{Time.now}!"
  end
 end
