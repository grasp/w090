...
class Rforum::Topic
  ...
  include Rforum::Mongoid::Likeable
  include Rforum::Mongoid::MarkdownBody
  include Redis::Objects
  include Rforum::Sunspot::Mongoid
  ...
  belongs_to :user, :inverse_of => :topics,:class_name=>"Ruser::User"
  counter_cache :name => :user, :inverse_of => :topics
  belongs_to :node ,:class_name=>"Rforum::Node"
  counter_cache :name => :node, :inverse_of => :topics
  belongs_to :last_reply_user, :class_name => 'Ruser::User'
  has_many :replies, :class_name => 'Rforum::Reply',:dependent => :destroy
